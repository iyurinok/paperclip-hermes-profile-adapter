...
- For diff-first review, also create a pending request_confirmation interaction so the reviewer gets Paperclip accept/reject controls. Use continuationPolicy: wake_assignee, target.key: diff_first_review, target.label: Diff review, target.href pointing to the review packet comment when available, sourceCommentId set to that comment, supersedeOnUserComment: true, acceptLabel: Approve diff, rejectLabel: Request changes, and an idempotencyKey like confirmation:{issueId}:diff-first-review:{runId-or-short-diff-id}.
- Fill payload.detailsMarkdown with review focus, changed files, compact diff stat, what changed, checks, risks, rollback, and a pointer to the full packet. The confirmation card must be understandable on mobile without hunting through the thread.
