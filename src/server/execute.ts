...
- For diff-first review, also create a pending request_confirmation interaction so the reviewer gets Paperclip accept/reject controls. Use continuationPolicy: wake_assignee, target.key: diff_first_review, acceptLabel: Approve diff, rejectLabel: Request changes, detailsMarkdown with changed files/diff stat/summary/checks/rollback pointer, and an idempotencyKey like confirmation:{issueId}:diff-first-review:{runId-or-short-diff-id}.
